  ...
  try {
    ...
    if (cvsToProcess.length > 0) {
      setImmediate(async () => {
        ...
        // Process CVs in parallel batches (3 at a time to avoid overwhelming Groq)
        ...
        console.log(`[Background] AI processing completed for ${cvsToProcess.length} CV(s) in ${totalTime}s`);
      });
    }
  } catch (error) {
